...
import "./libs/ISaitamaReferral.sol";
import "./interfaces/ISaitama.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

// MasterChef is the master of Saitama. He can make Saitama and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Saitama is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Saitama referral contract address.
    ISaitamaReferral public SaitamaReferral;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 SaitamaReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        // Saitama.mint(devAddress, SaitamaReward.div(10));

        Saitama.transferFrom(RewardWalletAddress, devAddress, SaitamaReward.div(10));

        // Saitama.mint(address(this), SaitamaReward);
    ...
    function deposit(
        ...
        if (
            _amount > 0 &&
            address(SaitamaReferral) != address(0) &&
            _referrer != address(0) &&
            _referrer != msg.sender
        ) {
            SaitamaReferral.recordReferral(msg.sender, _referrer);
        }
    ...
    // Update the Saitama referral contract address by the owner
    function setSaitamaReferral(ISaitamaReferral _SaitamaReferral) public onlyOwner {
        SaitamaReferral = _SaitamaReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(SaitamaReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = SaitamaReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(
                10000
            );

            if (referrer != address(0) && commissionAmount > 0) {
                // Saitama.mint(referrer, commissionAmount);
                ...
                SaitamaReferral.recordReferralCommission(referrer, commissionAmount);
